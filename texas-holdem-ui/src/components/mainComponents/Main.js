...
function Main(props) {
  ...
  const handleDealClick = () => {
    ...
    if(dealFlag=='hold'){
      ...
      let winner = callWinnerMethod1(playerList);
  ...
  function handRankingValue(handObject) {
    const rankings = ["High Card", "One Pair", "Two Pair", "Three of a Kind", "Straight", "Flush", "Full House", "Four of a Kind", "Straight Flush", "Royal Flush"];
    return rankings.indexOf(handObject.hand);
}
...
function callWinnerMethod1(playerList) {
    ...
    playerList.map(player => {
      let handValue = determineHandRanking(player.cards);
      playerHandList.push({
        player: player,
    ...
    winnerData.length > 0 && winnerData.map(winner => {
      console.log(winner.player.name + " " + winner.player_hand.hand);
    });
    return winnerData;
}
...
function callWinnerMethod2(playerList, flopCards) {
  let playerHandList = [];
  playerList.map(player => {
    let handValue = determineHandRanking([...player.cards, ...flopCards]);
    playerHandList.push({
      player: player,
      player_hand: handValue
    });
...
function callWinnerMethod3(playerList, flopCards, turnCards) {
  ...
  let playerHandList = [];
  playerList.map(player => {
    let handValue = determineHandRanking([...player.cards, ...allTableCards]);
    playerHandList.push({
      player: player,
      player_hand: handValue
    });
...
//    console.log(player.name+" "+action)
  ...
  function determineHandRanking(cards) {
    ...
    const rankOccurrences = Object.values(ranks);
    const isFourOfAKind = rankOccurrences.includes(4);
    const isFullHouse = rankOccurrences.includes(3) && rankOccurrences.includes(2);
    const isThreeOfAKind = rankOccurrences.includes(3);
    const isTwoPair = rankOccurrences.filter(val => val === 2).length === 2;
    const isOnePair = rankOccurrences.includes(2);

    const sortedByOccurrences = Object.entries(ranks).sort((a, b) => b[1] - a[1] || '23456789TJQKA'.indexOf(b[0]) - '23456789TJQKA'.indexOf(a[0]));

    if (isRoyalFlush) return { hand: "Royal Flush" };
    if (isStraightFlush) return { hand: "Straight Flush", highCard: sortedRanks[sortedRanks.length - 1] };
    if (isFourOfAKind) return { hand: "Four of a Kind", rank: sortedByOccurrences[0][0] };
    if (isFullHouse) return { hand: "Full House", threeRank: sortedByOccurrences[0][0], twoRank: sortedByOccurrences[1][0] };
    if (isFlush) return { hand: "Flush", highCard: sortedRanks[sortedRanks.length - 1] };
    if (isStraight) return { hand: "Straight", highCard: sortedRanks[sortedRanks.length - 1] };
    if (isThreeOfAKind) return { hand: "Three of a Kind", rank: sortedByOccurrences[0][0] };
    if (isTwoPair) return { hand: "Two Pair", highPair: sortedByOccurrences[0][0], lowPair: sortedByOccurrences[1][0] };
    if (isOnePair) return { hand: "One Pair", rank: sortedByOccurrences[0][0] };
    return { hand: "High Card", highCard: sortedRanks[sortedRanks.length - 1] };
}
