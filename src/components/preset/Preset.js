...
export const keys = () => {
  ...
  for (let i = 0; i < rank.length; i++) {
    ...
    for (let j = 0; j < rank.length; j++) {
      const rank2 = rank[j];
      let key =
        j > i ? rank1 + rank2 + SUIT.SUIT : rank2 + rank1 + SUIT.OFFSUIT;
      keyArr[i][j] = key;
    }
  }
  return keyArr;
...
export function createLowJack() {
  const table = createTable();
  return table;
}

export function createHighJack() {
  const table = createTable();
  return table;
}
...
export function mergeTables(table1, table2) {
  ...
  for (let i = 0; i < keyset.length; i++) {
    for (let j = 0; j < keyset[i].length; j++) {
      ...
      const value1 =
        table1[key] === undefined ? VALUE.FOLD : table1[key].toUpperCase();
      const value2 =
        table2[key] === undefined ? VALUE.FOLD : table2[key].toUpperCase();
      base[key] =
        value1 === VALUE.RAISE || value2 === VALUE.RAISE
          ? VALUE.RAISE
          : VALUE.FOLD;
    }
  }
