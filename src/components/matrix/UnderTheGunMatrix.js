...
// const UnderTheGunMatrix = ({ position, hand }) => {
...
//           if (item.player_type == "Ankit" && item.position == "UTG") {
...
// export default UnderTheGunMatrix;
...
const UTGMatrix = ({ position, hand, data }) => {
  const [selected, setSelected] = useState(
    Array(13)
      .fill()
      .map(() => Array(13).fill(false))
  ...
  const [table_utg, setTable_utg] = useState(createUTG());
  ...
  useEffect(() => {
    ...
    setTable("utg", setTable_utg, createUTG);
  }, [data]);
                  ...
                  {keyTable[i].map((key, j) => {
                    return (
                      <td
                        key={position + "-" + key}
                        className={
                          table_utg[key] === VALUE.RAISE ? "selected" : ""
                        }
                        onMouseDown={() => handleMouseDown(i, j)}
