...
// const BigBlindMatrix = ({ position, hand }) => {
...
//           if (item.player_type == "Ankit" && item.position == "big-blind") {
...
// export default BigBlindMatrix;

import React, { useState, useEffect } from "react";
import "./Matrix.css";
import axios from "axios";
import { VALUE, createBigBlind } from "../preset/Preset.js";
import "./Table.css";
import {
  createUTG,
  createUTGp1,
...
const BigBlindMatrix = ({ position, hand, data }) => {
  const [selected, setSelected] = useState(
    Array(13)
      .fill()
      .map(() => Array(13).fill(false))
  ...
  const [table_bb, setTable_bb] = useState(createBigBlind());
  const [draggingState, setDraggingState] = useState(false);
  ...
  useEffect(() => {
    function setTable(pos, setState, standardTable) {
      if (data.length > 0) {
        ...
        data.map((preflop) => {
          if (
            preflop.player_type === "AM" &&
            preflop.position === "big-blind"
          ) {
            let playerTable;
            try {
    ...
    setTable("bb", setTable_bb, createBigBlind);
  }, [data]);
                  ...
                  {keyTable[i].map((key, j) => {
                    return (
                      <td
                        key={position + "-" + key}
                        className={
                          table_bb[key] === VALUE.RAISE ? "selected" : ""
                        }
                        onMouseDown={() => handleMouseDown(i, j)}
