...
// const CutOffMatrix = ({ position, hand }) => {
...
//           if (item.player_type == "Ankit" && item.position == "cut-off") {
...
const CutOffMatrix = ({ position, hand, data }) => {
  const [selected, setSelected] = useState(
    Array(13)
      .fill()
      .map(() => Array(13).fill(false))
    ...
    axios
      ...
      .then((res) => {
        ...
        res.data.map((item) => {
          //                console.log(item.player_type+" "+item.position)
          // use cut-off instead of co later on
          if (item.player_type == "Ankit" && item.position == "co") {
            //console.log("Im in "+item.preflop_table)
            var obj = item.preflop_table;
                  ...
                  {keyTable[i].map((key, j) => {
                    return (
                      <td
                        key={position + "-" + key}
                        className={
                          table_co[key] === VALUE.RAISE ? "selected" : ""
                        }
                        onMouseDown={() => handleMouseDown(i, j)}
