...
// const LowJackMatrix = ({ position, hand }) => {
...
//           if (item.player_type == "Ankit" && item.position == "low-jack") {
...
// export default LowJackMatrix;

import React, { useState, useEffect } from "react";
import "./Matrix.css";
import axios from "axios";
import { VALUE, createLowJack } from "../preset/Preset.js";
import "./Table.css";
import {
  createUTG,
  createUTGp1,
...
const LowJackMatrix = ({ position, hand, data }) => {
  const [selected, setSelected] = useState(
    Array(13)
      .fill()
      .map(() => Array(13).fill(false))
  ...
  const [table_lj, setTable_lj] = useState(createLowJack());
  const [draggingState, setDraggingState] = useState(false);
  ...
  useEffect(() => {
    ...
    setTable("lj", setTable_lj, createLowJack);
  }, [data]);
                  ...
                  {keyTable[i].map((key, j) => {
                    return (
                      <td
                        key={position + "-" + key}
                        className={
                          table_lj[key] === VALUE.RAISE ? "selected" : ""
                        }
                        onMouseDown={() => handleMouseDown(i, j)}
