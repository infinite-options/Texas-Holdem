...
// const HighJackMatrix = ({ position, hand }) => {
...
//           if (item.player_type == "Ankit" && item.position == "high-jack") {
...
// export default HighJackMatrix;

import React, { useState, useEffect } from "react";
import "./Matrix.css";
import axios from "axios";
import { VALUE } from "../preset/Preset.js";
import "./Table.css";
import {
  createUTG,
  createUTGp1,
...
const HighJackMatrix = ({ position, hand, data }) => {
  const [selected, setSelected] = useState(
    Array(13)
      .fill()
      .map(() => Array(13).fill(false))
  ...
  const [table_hj, setTable_hj] = useState(createHighJack());
  ...
  useEffect(() => {
    ...
    setTable("hj", setTable_hj, createHighJack);
  }, [data]);
                  ...
                  {keyTable[i].map((key, j) => {
                    return (
                      <td
                        key={position + "-" + key}
                        className={
                          table_hj[key] === VALUE.RAISE ? "selected" : ""
                        }
                        onMouseDown={() => handleMouseDown(i, j)}
