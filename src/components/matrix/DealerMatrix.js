...
// const DealerMatrix = ({position, hand}) => {
...
//             axios.post('http://127.0.0.1:5000/insert', input) // Sending data to backend
//             .then(res => console.log(res))
//             .catch(err => console.error(err));
//     };
...
//         axios.get('https://4jzrko6ioa.execute-api.us-west-1.amazonaws.com/dev/api/v2/preflop') // Sending data to backend
//         .then(res => {
//            // console.log(res)
//             res.data.map( item =>{
// //                console.log(item.player_type+" "+item.position)
//                 // use cut-off instead of co later on  
//                 if(item.player_type=='Ankit' && item.position=='dealer'){
...
// export default DealerMatrix;

import React, { useState, useEffect } from "react";
import "./Matrix.css";
import axios from "axios";
import { VALUE, createDealer } from "../preset/Preset.js";
import "./Table.css";
import {
  createUTG,
  createUTGp1,
...
const DealerMatrix = ({ position, hand, data }) => {
  const [selected, setSelected] = useState(
    Array(13)
      .fill()
  ...
  const [table_dealer, setTable_dealer] = useState(createDealer());
  const [draggingState, setDraggingState] = useState(false);
  ...
  useEffect(() => {
    ...
    setTable("dealer", setTable_dealer, createDealer);
  }, [data]);
    ...
    axios
      .post(
          ...
          "/preflop",
        {
          player_type: "Ankit",
          position: position,
          preflop_table: JSON.stringify(table_dealer),
        }
      )
      .then((res) => {
        console.log(res);
  ...
  return (
    <div>
      ...
      {reference !== "EMPTY" ? (
        <div>
          {/* <JTable data={[keyTable,position,table_dealer]}></JTable> */}
                  ...
                  {keyTable[i].map((key, j) => {
                    return (
                      <td
                        key={position + "-" + key}
                        className={
                          table_dealer[key] === VALUE.RAISE ? "selected" : ""
                        }
                        onMouseDown={() => handleMouseDown(i, j)}
                        onMouseOver={() => handleMouseOver(i, j)}>
                        {" "}
